...
const getAdjacent = (state, pos) => {
  ...
  if (row > 0) result.push(pos - state.boardSize)
  if (row < state.boardSize - 1) result.push(pos + state.boardSize)
  if (col > 0) result.push(pos - 1)
  if (col < state.boardSize - 1) result.push(pos + 1)
  return result
}
...
const validateWin = (state) => {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (state.gridData[i] !== target) return false
  }
  return true
}
...
export default {
  ...
  getters: {
    ...
    isWin: (state) => state.gridData.length ? validateWin(state) : false,
    ...
    secondsUntilNextFree: (state, getters) => {
      if (state.gameCompleted || !state.gridData.length || getters.isWin) return null
  ...
  mutations: {
    [MUTATIONS.SET_GRID_DATA]: (state, payload) => { state.gridData = payload },
    [MUTATIONS.SET_BOARD_SIZE]: (state, payload) => { state.boardSize = payload },
    [MUTATIONS.SET_STEP_COUNT]: (state, payload) => { state.stepCount = payload },
    [MUTATIONS.SET_RANDOM_BLOCK_MODE]: (state, payload) => { state.randomBlockMode = payload },
    [MUTATIONS.SET_FREEZE_MODE]: (state, payload) => { state.freezeMode = payload },
    [MUTATIONS.SET_BLOCKED_POSITION]: (state, payload) => { state.blockedPosition = payload },
    [MUTATIONS.SET_ELAPSED_SECONDS]: (state, payload) => { state.elapsedSeconds = payload },
    [MUTATIONS.SET_FREE_MOVE_TOKENS]: (state, payload) => { state.freeMoveTokens = payload },
    [MUTATIONS.SET_FREE_MOVE_TICK]: (state, payload) => { state.freeMoveTick = payload },
    [MUTATIONS.SET_GAME_COMPLETED]: (state, payload) => { state.gameCompleted = payload },
    [MUTATIONS.SET_RECORDS]: (state, payload) => { state.records = { ...payload } },
    [MUTATIONS.SET_MOVE_HISTORY]: (state, payload) => { state.moveHistory = payload },
    [MUTATIONS.SET_LAST_MOVE_TIME]: (state, payload) => { state.lastMoveTime = payload },
    [MUTATIONS.SET_FAST_FORWARD]: (state, payload) => { state.isFastForward = payload },
    [MUTATIONS.ADD_PENALTY_TIME]: (state, payload) => { state.elapsedSeconds += payload },
    [MUTATIONS.SET_FROZEN_TILES]: (state, tiles) => { state.frozenTiles = tiles },
  ...
  actions: {
    loadRecordsFromStorage ({ commit }) {
      let raw = null
      try { raw = window.localStorage.getItem(STORAGE_KEY_PUZZLE_RECORDS) } catch (e) { raw = null }
      if (!raw) { commit(MUTATIONS.SET_RECORDS, {}); return }
      let parsed = null
      try { parsed = JSON.parse(raw) } catch (e) { parsed = {} }
      if (!parsed || typeof parsed !== 'object') { commit(MUTATIONS.SET_RECORDS, {}); return }
      commit(MUTATIONS.SET_RECORDS, parsed)
    },
    persistRecords ({ state }) {
      try { window.localStorage.setItem(STORAGE_KEY_PUZZLE_RECORDS, JSON.stringify(state.records)) } catch (e) {}
    },
    saveBestTimeIfNeeded ({ state, dispatch, commit, getters }) {
      if (!getters.isWin) return
      const key = String(state.boardSize)
      const prev = state.records[key]
      const t = state.elapsedSeconds
      if (prev !== undefined && prev !== null && t >= prev) return
      const next = { ...state.records, [key]: t }
      commit(MUTATIONS.SET_RECORDS, next)
      dispatch('persistRecords')
    },
    rollBlockedNeighbor ({ state, commit }) {
      if (!state.randomBlockMode) { commit(MUTATIONS.SET_BLOCKED_POSITION, null); return }
      const voidPos = state.gridData.indexOf(0)
      commit(MUTATIONS.SET_BLOCKED_POSITION, pickRandomBlockedNeighbor(state, voidPos))
    },
    ...
    mixBoard ({ state, dispatch }) {
      ...
      for (let i = 0; i < mixSteps; i++) {
        const voidPos = state.gridData.indexOf(0)
        const adj = getAdjacent(state, voidPos)
        const allowed = adj.filter((n) => n !== lastPos)
    ...
    setBoardSize ({ commit, dispatch }, nextSize) {
      if (nextSize < minBoardSize) return
    ...
    tickSecond ({ state, commit, getters }) {
      if (state.gameCompleted || getters.isWin) return
      const now = Date.now()
      ...
      if (nextTick >= FREE_MOVE_INTERVAL_SECONDS) {
        ...
        commit(MUTATIONS.SET_FREE_MOVE_TICK, nextTick - FREE_MOVE_INTERVAL_SECONDS)
      } else {
    ...
    tryMoveTile ({ state, getters, dispatch }, pos) {
      if (state.gameCompleted || getters.isWin) return
      const voidPos = state.gridData.indexOf(0)
      if (pos === voidPos) return
      if (state.frozenTiles.includes(pos)) return
      if (state.frozenTiles.includes(voidPos)) return
      
      const tileValue = state.gridData[pos]
      if (state.freezeMode && tileValue !== 0) {
        const correctPosition = tileValue - 1
        if (pos === correctPosition) return
      }
      ...
      if (state.freeMoveTokens <= 0) return
    ...
    applyMove ({ state, dispatch, commit, getters }, { voidPos, pos, useFreeToken }) {
      ...
      if (useFreeToken) commit(MUTATIONS.SET_FREE_MOVE_TOKENS, Math.max(0, state.freeMoveTokens - 1))
      dispatch('rollBlockedNeighbor')
      if (getters.isWin) {
    ...
    resizeBoardDelta ({ state, dispatch }, delta) {
      dispatch('setBoardSize', state.boardSize + delta)
    },
  },
}